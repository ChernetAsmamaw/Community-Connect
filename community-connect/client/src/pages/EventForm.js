...
const EventForm = ({ start, end, onSubmit, onCancel }) => {
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [date, setDate] = useState(start.toISOString().substring(0, 10));
  const [startTime, setStartTime] = useState(start.toISOString().substring(11, 16));
  const [endTime, setEndTime] = useState(end.toISOString().substring(11, 16));
